        ...
        children: [
          Container(
            ...
            child: Row(
              children: [
                    ...
                    child: TextField(
                      ...
                      onChanged: (val) {
                        setState(() {
                          ...
                          filteredConversations = widget.conversations.where((conv) {
                            final members = (conv['members'] as List?) ?? [];
                            dynamic otherUser;
                            try {
                              otherUser = members.firstWhere(
                                    (m) => (m is Map ? m['_id'].toString() : m.toString()) != widget.userId,
                                orElse: () => null,
                              );
                            } catch (e) {
                              print("Abhi:- Error finding other user for filtering: $e");
                              return false;
                            }
                            if (otherUser == null) return false;
                            final name = otherUser is Map ? (otherUser['full_name'] ?? '').toLowerCase() : '';
                ...
                //   child: IconButton(
                //     onPressed: () {
                //       print("Abhi:- Start new conversation button pressed");
                //       showDialog(
                //         context: context,
                //         builder: (ctx) {
                ...
                //                     widget.onReceiverIdChanged(idController.text);
                //                     widget.onStartConversation();
                //                   }
                //                   Navigator.pop(ctx);
                //                 },
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (ctx, idx) {
                final conv = filteredConversations[idx];
                final members = (conv['members'] as List?) ?? [];
                dynamic otherUser;
                try {
                  otherUser = members.firstWhere(
