...
//   final String userId;
//
//   const ChatDetailScreen({Key? key, required this.currentChat, required this.userId}) : super(key: key);
//
//   @override
...
//       if (parsedMessage != null && parsedMessage['conversationId'] != null) {
//         if (parsedMessage['conversationId'] == widget.currentChat['_id']) {
//           setState(() {
//             if (!messages.any((msg) => msg['_id'] == parsedMessage['_id'])) {
//               messages.add(parsedMessage);
//               // Sort messages to ensure newest is at the end
...
//         }
//       } else {
//         print('Abhi:- ❌ Invalid message data: $parsedMessage');
//       }
//     });
//     SocketService.listenOnlineUsers((users) {
...
//   Future<void> _fetchMessages() async {
//     try {
//       final msgs = await ApiService.fetchMessages(widget.currentChat['_id']);
//       setState(() {
//         // Filter out duplicates and sort by createdAt (newest last)
//         messages = msgs
//             .where((msg) => !messages.any((m) => m['_id'] == msg['_id']))
//             .toList()
//           ..sort((a, b) => DateTime.parse(a['createdAt']).compareTo(DateTime.parse(b['createdAt'])));
//       });
//       print('Abhi:- Messages fetched, total: ${messages.length}, scrolling to today\'s latest');
//       _scrollToBottom();
//     } catch (e) {
...
//       return;
//     }
//     final receiverId = widget.currentChat['members']
//         ?.firstWhere((m) => m['_id'] != widget.userId, orElse: () => null)?['_id'];
//     if (receiverId == null || widget.currentChat == null) {
//       print('Abhi:- Current chat or receiverId is null, cannot send message.');
//       ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Pehle chat select karo!')));
//       return;
//     }
...
//           receiverId: receiverId,
//           conversationId: widget.currentChat['_id'],
//           imagePaths: images,
//           message: _messageController.text.isNotEmpty ? _messageController.text : null,
//         );
//         print('Abhi:- Image message sent successfully: $newMsg');
...
//   void _scrollToBottom() {
//     if (_scrollController.hasClients) {
//       // Find the index of the latest message from today
//       final now = DateTime.now();
//       final today = DateTime(now.year, now.month, now.day);
//       int latestTodayIndex = -1;
//       for (int i = messages.length - 1; i >= 0; i--) {
//         final msgDate = DateTime.parse(messages[i]['createdAt']).toLocal();
...
//         // Scroll to the latest message from today
//         print('Abhi:- Scrolling to today\'s latest message at index: $latestTodayIndex');
//         _scrollController.jumpTo(
//           latestTodayIndex * 100.0, // Approximate height per message, adjust if needed
//         );
//       } else {
//         // If no messages from today, scroll to the end
//         print('Abhi:- No messages from today, scrolling to end, maxScrollExtent: ${_scrollController.position.maxScrollExtent}');
//         _scrollController.jumpTo(_scrollController.position.maxScrollExtent);
//       }
//     } else {
//       print('Abhi:- ScrollController not attached yet');
...
//
//           if (latestTodayIndex != -1) {
//             print('Abhi:- Post-frame scrolling to today\'s latest message at index: $latestTodayIndex');
//             _scrollController.jumpTo(
//               latestTodayIndex * 100.0, // Approximate height per message, adjust if needed
//             );
//           } else {
//             print('Abhi:- Post-frame scrolling to end');
//             _scrollController.jumpTo(_scrollController.position.maxScrollExtent);
//           }
//         }
//       });
...
//   @override
//   Widget build(BuildContext context) {
//     final members = (widget.currentChat['members'] as List);
//     final otherUser = members.firstWhere(
...
//                           children: [
//                             if (msg['messageType'] == 'image' && msg['image'] != null)
//                               ...msg['image'].map<Widget>((imgUrl) => GestureDetector(
//                                 onTap: () {
//                                   Navigator.push(
//                                     context,
//                                     MaterialPageRoute(
//                                       builder: (context) => FullImageScreen(
//                                         imageUrl: 'https://api.thebharatworks.com/$imgUrl',
//                                       ),
//                                     ),
//                                   );
//                                 },
...
//                               )),
//                             if (msg['messageType'] == 'document' && msg['document'] != null)
//                               ...msg['document'].map<Widget>((docUrl) => GestureDetector(
//                                 onTap: () {
//                                   print('Abhi:- Opening document: https://api.thebharatworks.com/$docUrl');
//                                   // Add real open logic here
//                                 },
//                                 child: Container(
//                                   margin: EdgeInsets.only(bottom: 8),
//                                   padding: EdgeInsets.all(8),
