...
class _UserViewWorkerDetailsState extends State<UserViewWorkerDetails> {
  ...
  String? getFullImageUrl(String? path) {
    if (path == null || path.isEmpty) {
      print("Image URL is null or empty: $path");
      return null; // Null return karo taaki NetworkImage use na ho
    }
    if (path.startsWith("http")) {
      print("Image URL already has host: $path");
      return path;
    }
    final url = "https://api.thebharatworks.com$path";
        ...
        if (data['status'] == true && data['data'] != null) {
          setState(() {
            ...
            if (workerData?.fullAddress != null && workerData!.fullAddress!.isNotEmpty) {
              ...
              userLocation = workerData?.location?['address'] ?? 'Location not available';
            }
            isLoading = false;
          });
        } else {
