...
class _ServiceDisputeScreenState extends State<ServiceDisputeScreen> {
  ...
  // 🚀 Submit dispute to API with multiple images
  Future<void> submitDispute() async {
    ...
    try {
      ...
      for (var image in _selectedImages) {
        request.files.add(
          await http.MultipartFile.fromPath('images', image.path), // <-- yaha "images" likh
        );
      }
  ...
  // 🧾 Input TextField
  Widget _buildTextField(TextEditingController controller, keybordType, {int maxLines = 1}) {
    return Container(
      width: 320,
