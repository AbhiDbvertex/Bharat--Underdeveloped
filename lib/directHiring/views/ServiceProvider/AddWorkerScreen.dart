...
class _AddWorkerScreenState extends State<AddWorkerScreen> {
  ...
  String? _validateDate(String? value) {
    ...
    try {
      ...
      final now = DateTime.now();
      final earliest = DateTime(1900);
      if (date.isAfter(now)) return 'Bhai, future ka date nahi chalega';
      if (date.isBefore(earliest)) return 'Itna purana date bhi nahi';
      final formatted =
          '${date.year}-${date.month.toString().padLeft(2, '0')}-${date.day.toString().padLeft(2, '0')}';
      if (formatted != value) return 'Date galat hai, check kar';
      return null;
    } catch (e) {
      return 'Date format sahi nahi hai';
    }
  }
