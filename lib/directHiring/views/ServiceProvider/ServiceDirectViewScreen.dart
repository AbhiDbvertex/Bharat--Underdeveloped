...
class ServiceDirectViewScreen extends StatefulWidget {
  ...
  @override
  State<ServiceDirectViewScreen> createState() => _ServiceDirectViewScreenState();
}
...
class _ServiceDirectViewScreenState extends State<ServiceDirectViewScreen> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (decoded['data']?['order']?['offer_history'] != null &&
          ...
          subcategoryId = decoded['data']['order']['offer_history'][0]['provider_id']['subcategory_ids']?.isNotEmpty == true
              ? decoded['data']['order']['offer_history'][0]['provider_id']['subcategory_ids'][0]['_id']?.toString()
              : null;
          print("🚫 Provider ID mil gaya: $providerId");
          print("📋 Category ID: $categoryId, Subcategory ID: $subcategoryId");
        } else {
        ...
        if (categoryId != null && subcategoryId != null) {
          await fetchServiceProvidersListinWorkDetails(categoryId, subcategoryId);
        } else {
          print("⚠️ Category ID ya Subcategory ID nahi mila, skipping fetchServiceProviders!");
          _showSnackBar("Category or subcategory ID missing!");
        }
      } else {
        print("❌ API ne dhoka diya: ${response.statusCode} - ${response.body}");
  ...
  Future<void> _clearHiredProviders() async {
    ...
    if (offerHistory != null && offerHistory.isNotEmpty) {
      final providerStatus = offerHistory[0]['status']?.toString().toLowerCase();
      print("📩 Provider Status in Offer History: $providerStatus");
      if (providerStatus == 'rejected' || providerStatus == 'cancelled') {
        print("⚠️ Provider ne offer pehle se reject kiya hai!");
        if (mounted) {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://api.thebharatworks.com/api/direct-order/reject-offer'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
      ...
      if (response.statusCode == 200) {
        ...
        final err = json.decode(response.body);
        print("❌ API Error: ${response.statusCode} - ${err['message']}");
        if (err['message'] == "Invalid rejection or already handled" && mounted) {
          _showSnackBar("Order already rejected or invalid!");
          Navigator.pop(context);
        } else {
          _showSnackBar("Error ${response.statusCode}: ${err['message']}");
      ...
      if (order!['service_payment'] != null && order!['service_payment']['payment_history']?.isNotEmpty == true) {
        try {
          final refundResponse = await http.post(
  ...
  Future<void> fetchServiceProvidersListinWorkDetails(String? categoryId, String? subCategoryId) async {
    ...
    final uri = Uri.parse('${AppConstants.baseUrl}${ApiEndpoint.serviceDirectViewScreen}');
    try {
      final response = await http.post(
        uri,
        headers: {
        ...
        if (data["status"] == true && data["data"] != null) {
          setState(() {
            providerslist = (data["data"] as List)
                .map((json) {
              final provider = ServiceProviderModel.fromJson(json);
              bool isExcluded = hiredProviderIds.contains(provider.id) || provider.id == orderProviderId;
              print("🔍 Provider ID: ${provider.id}, Name: ${provider.fullName}, Excluded: $isExcluded");
              return provider;
            })
                .where((provider) {
              bool isValid = provider.id != null && !hiredProviderIds.contains(provider.id) && provider.id != orderProviderId;
              if (!isValid) {
                print("🚫 Filtered out provider: ${provider.id}");
              }
              return isValid;
  ...
  Future<void> markOrderAsComplete() async {
    try {
      ...
      var response = await http.post(
        Uri.parse("https://api.thebharatworks.com/api/direct-order/completeOrderProvider"),
        headers: headers,
        body: body,
      );
  ...
  Future<void> acceptOffer(String orderId) async {
    ...
    try {
      ...
      print("📤 OrderId: $orderId ke liye accept request bhej raha hoon");
      final response = await http.post(
        Uri.parse('https://api.thebharatworks.com/api/direct-order/accept-offer'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
      ...
      if (response.statusCode == 200) {
        ...
        final err = json.decode(response.body);
        _showSnackBar("Error ${response.statusCode}: ${err['message']}");
        print("❌ API ka dimaag kharab: ${response.statusCode} - ${err['message']}");
        if (err['message'] == "Invalid acceptance or already handled") {
          setState(() {
            _isOrderAccepted = true;
            order!['hire_status'] = 'accepted';
  ...
  @override
  Widget build(BuildContext context) {
    print("📋 DirectViewScreen categreyId: ${widget.categreyId}, subCategreyId: ${widget.subcategreyId}");
    return Scaffold(
      appBar: AppBar(
        backgroundColor: AppColors.primaryGreen,
        centerTitle: true,
      ...
      body: isLoading
        ...
        child: Column(
          ...
          children: [
            ...
            order!['image_url'] != null && (order!['image_url'] is List && order!['image_url'].isNotEmpty)
                ? CarouselSlider(
              ...
              items: (order!['image_url'] as List).map((imageUrl) {
                final url = imageUrl.startsWith('http') ? imageUrl : 'https://api.thebharatworks.com$imageUrl';
            ...
            if (order?['hire_status']?.toLowerCase() == 'pending') ...[
              GestureDetector(
                onTap: _isProcessing || _isOrderAccepted
                  ...
                  String orderId = order!['_id'].toString();
                  print("✅ OrderId $orderId ke liye Accept dabaya!");
                  acceptOffer(orderId);
                },
                child: Container(
                  width: double.infinity,
                  height: 50,
                  decoration: BoxDecoration(
                    color: _isOrderAccepted ? Colors.grey.shade400 : Colors.green.shade700,
                    borderRadius: BorderRadius.circular(12),
                    boxShadow: [
                      BoxShadow(
                        color: Colors.grey.withOpacity(0.1),
                        ...
                        children: [
                          CircleAvatar(
                            ...
                            backgroundImage: order != null &&
                                ...
                                ? NetworkImage(order!['assignedWorker']['image'])
                                : AssetImage('assets/profile_pic.jpg'),
                            onBackgroundImageError: (exception, stackTrace) {
                              print("📸 Assigned Worker Image Error: $exception");
                            },
                          ),
                          SizedBox(width: 30),
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(
                                  order != null && order!['assignedWorker'] != null
                                ...
                                GestureDetector(
                                  onTap: () {
                                    if (order != null &&
                                      ...
                                      print("❌ Assigned worker ID not found");
                                      _showSnackBar("Worker data missing!");
                                    }
                                  },
                                  child: Container(
                                    height: 30,
                ...
                GestureDetector(
                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => ServiceWorkerListScreen(orderId: widget.id),
                      ),
                    ).then((_) {
                      fetchOrderDetail();
                    });
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 10),
                      if (order!['service_payment'] != null &&
                          order!['service_payment']['payment_history'] != null &&
                          order!['service_payment']['payment_history'].isNotEmpty)
                        ...List.generate(
                          order!['service_payment']['payment_history'].length,
                              (index) {
                            final item = order!['service_payment']['payment_history'][index];
                            return Padding(
