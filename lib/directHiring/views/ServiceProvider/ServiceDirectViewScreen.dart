...
//   bool _isOrderAccepted = false; // To track if order is accepted
//
//   @override
...
//       print("📋 fetchOrderDetail token: $token");
//
//       if (token.isEmpty) {
...
//         print("📥 fetchOrderDetail response: ${response.body}");
//         String? providerId;
...
//         );
//         setState(() => isLoading = false);
//         _showSnackBar("Order details nahi mile!");
//         await fetchServiceProvidersListinWorkDetails(
//           widget.categreyId,
//           widget.subcategreyId,
//         );
...
//       print("❌ Error: Order or orderId is null");
//       _showSnackBar("Bhai, order ka data missing hai!");
//       return;
//     }
//
...
//       print("⚠️ Order already rejected or cancelled");
//       if (mounted) {
...
//         print("⚠️ Provider already rejected this offer");
//         if (mounted) {
...
//           _showSnackBar("Offer rejected successfully");
//           Navigator.pop(context);
//         } else {
//           print("❌ Reject Failed: ${data['message']}");
//           _showSnackBar("Failed to reject offer: ${data['message']}");
//         }
//       } else {
...
//
//     if (token == null) {
//       _showSnackBar("Bhai, pehle login kar!");
//       return;
//     }
//
//     if (categoryId == null || subCategoryId == null) {
//       print(
//         "⚠️ categoryId ya subCategoryId null hai, API call skip kar raha hoon",
//       );
//       return;
//     }
//
...
//           });
//         } else {
//           _showSnackBar("Koi providers nahi mile!");
//         }
//       } else {
//         final err = json.decode(response.body);
//         _showSnackBar("Error ${response.statusCode}: ${err['message']}");
...
//   //     if (token == null) {
//   //       _showSnackBar("Bhai, pehle login toh kar!");
//   //       return;
//   //     }
//   //
//   //     var headers = {
...
//   //
//   //     if (response.statusCode == 200) {
//   //       _showSnackBar("Order complete mark ho gaya!");
//   //       Navigator.pop(context); // Navigate back after success
//   //     } else {
//   //       var data = jsonDecode(response.body);
//   //       _showSnackBar("Complete mark nahi hua: ${data['message']}");
//   //     }
//   //   } catch (e) {
//   //     _showSnackBar("Error: $e");
//   //   }
...
//
//       var body = jsonEncode({"order_id": widget.id});
//       print(" anjali  Request Body: $body"); // 👈 Body print karo
//
//       var response = await http.post(
//         Uri.parse(
...
//
//       if (response.statusCode == 200) {
//         _showSnackBar("Mark as complete Successfully!");
//         Navigator.pop(context); // Navigate back after success
//       } else {
//         var data = jsonDecode(response.body);
//         _showSnackBar("Complete mark nahi hua: ${data['message']}");
...
//
//       if (_isOrderAccepted) {
//         print("✅ Order pehle se accept hai, bas UI dikhayenge!");
//         setState(() => _isProcessing = false);
//         return; // No API call, just show the UI
//       }
//
...
//           setState(() {
//             _isOrderAccepted = true; // Update state to show WorkDetail UI
//             order!['hire_status'] = 'accepted'; // Update local order status
//           });
//           _showSnackBar("Offer accept ho gaya!");
//         } else {
//           _showSnackBar("Offer accept nahi hua: ${data['message']}");
...
//       }
//     } catch (e) {
//       print("❗ AcceptOffer mein gadbad: $e");
//       _showSnackBar("Error: $e");
//     } finally {
//       if (mounted) {
//         setState(() => _isProcessing = false);
...
//                   if (order == null || order!['_id'] == null) {
//                     print(
//                       "❌ Arre bhai, order ya orderId toh hai hi nahi!",
//                     );
//                     _showSnackBar(
//                       "Bhai, order ka data gayab hai!",
//                     );
//                     return;
//                   }
//                   String orderId = order!['_id'].toString();
//                   print(
//                     "✅ Chalo, orderId $orderId ke liye Accept dabaya!",
//                   );
//                   acceptOffer(orderId);
//                 },
//                 child: Container(
...
//                 Text(
//                   "Arre bhai, tu toh is order ko pehle hi accept kar chuka hai!",
//                   style: GoogleFonts.roboto(
//                     fontSize: 14,
//                     color: Colors.grey,
//                     fontWeight: FontWeight.w500,
...
//             ],
//             if (_isOrderAccepted) ...[
//               // ServiceWorkDetailScreen UI starts here
//               Container(
//                 height: 120,
//                 width: double.infinity,
//                 decoration: BoxDecoration(
...
//                               null
//                           ? Image.network(
//                         'https://api.thebharatworks.com${order!['user_id']['profile_pic']}',
//                         width: 80,
//                         height: 80,
//                         fit: BoxFit.cover,
//                         errorBuilder: (
...
//                             ) {
//                           print(
//                             "📸 Profile pic load mein gadbad: $error",
//                           );
//                           return Container(
//                             width: 80,
//                             height: 80,
...
//                             children: [
//                               Text(
//                                 "Project Fees – ₹${order != null && order!['service_payment'] != null ? order!['service_payment']['total_expected'] ?? '200' : '200'}/-",
//                                 style: GoogleFonts.roboto(
//                                   color: Colors.black,
//                                   fontSize: 12,
//                                   fontWeight: FontWeight.w500,
...
class _ServiceDirectViewScreenState extends State<ServiceDirectViewScreen> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (categoryId != null && subcategoryId != null) {
          ...
          _showSnackBar("Bhai, category ya subcategory ID missing hai!");
        }
      } else {
        print("❌ API ne dhoka diya: ${response.statusCode} - ${response.body}");
        setState(() => isLoading = false);
  ...
  Future<void> _clearHiredProviders() async {
    ...
    if (orderStatus == 'rejected' || orderStatus == 'cancelled') {
      print("⚠️ Order pehle se reject ya cancel hai!");
      if (mounted) {
        _showSnackBar("Order already rejected");
        Navigator.pop(context);
      }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        if (data['message'] == "Offer rejected successfully" && mounted) {
          _showSnackBar("Offer reject ho gaya!");
          Navigator.pop(context);
        } else {
          print("❌ Reject Failed: ${data['message']}");
          _showSnackBar("Reject nahi hua: ${data['message']}");
        }
      } else {
        final err = json.decode(response.body);
        print("❌ API Error: ${response.statusCode} - ${err['message']}");
        if (err['message'] == "Invalid rejection or already handled" &&
            mounted) {
          _showSnackBar("Order already rejected ya invalid hai!");
          Navigator.pop(context);
        } else {
          _showSnackBar("Error ${response.statusCode}: ${err['message']}");
        }
  ...
  Future<void> markOrderAsComplete() async {
    try {
      ...
      if (response.statusCode == 200) {
        _showSnackBar("Mark as complete ho gaya!");
        Navigator.pop(context);
      } else {
        var data = jsonDecode(response.body);
        _showSnackBar("Complete mark nahi hua: ${data['message']}");
